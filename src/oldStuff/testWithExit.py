...
class SimulationEngine:
    def __init__(self, service_rate, max_visits=2):
        ...
        self.visit_counter = {}  # personID -> visits
    ...
    def initialize(self, person_arrivals):
        for person, arrival_time in person_arrivals:
            ...
            self.visit_counter[person.ID] = 0
    ...
    def run(self):
        while not self.event_queue.is_empty():
            ...
            if event.event_type == 'ARRIVAL':
                ...
                self.visit_counter[person.ID] += 1
                if self.visit_counter[person.ID] < self.max_visits:
                    delay = 0.5
                    next_arrival_time = self.current_time + delay
                    self.schedule_event(next_arrival_time, 'ARRIVAL', person)
                else:
                    print(f"[{self.current_time:.3f}] Person {person.ID} LEAVES the system after {self.visit_counter[person.ID]} visits")
...
def Main():
    ...
    sim = SimulationEngine(service_rate, max_visits=2)
