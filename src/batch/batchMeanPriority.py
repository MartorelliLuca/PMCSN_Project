...
NOTE: fare attenzione al formato del json priorita/non priorita
"""
...
def read_stats(file_path, n):
    """
    Legge i dati dai file JSON giornalieri con supporto per code multiple di InValutazione.
    Per ogni servizio, crea liste di valori per queue_time, service_time, response_time.
    Gestisce la nuova struttura di InValutazione con tre code separate.
        ...
        for line in f:
            ...
            for service_name, service_stats in stats.items():
                if service_name == "InValutazione":
                    ...
                    if 'visited' in service_stats and isinstance(service_stats['visited'], dict):
                        # Extract sub-queues: Leggera, Diretta, Pesante
                        sub_queues = service_stats['visited'].keys()
                        for sub_queue in sub_queues:
                            sub_service_name = f"InValutazione_{sub_queue}"
                            
                            # Get queue time and execution time for this sub-queue
                            queue_time = service_stats['queue_time'].get(sub_queue, 0)
                            exec_time = service_stats['executing_time'].get(sub_queue, 0)
                            response_time = queue_time + exec_time
...
if __name__ == "__main__":
    ...
    stats = read_daily_stats('transient_analysis_json/daily_stats.json')
    
    # Define services including InValutazione sub-queues
    basic_centers = ["Autenticazione", "Instradamento", "CompilazionePrecompilata", "InvioDiretto"]
    invalutazione_centers = ["InValutazione_Leggera", "InValutazione_Diretta", "InValutazione_Pesante"]
    centers = basic_centers + invalutazione_centers
    ...
    for row in stats:
        ...
        # Process basic centers (old structure)
        for center in basic_centers:
            if center in row_stats:
        ...
        # Process InValutazione sub-queues (new structure)
        if "InValutazione" in row_stats:
            invalutazione_stats = row_stats["InValutazione"]
            if isinstance(invalutazione_stats.get('visited'), dict):
                # New structure with sub-queues
                for sub_queue in invalutazione_stats['visited'].keys():
                    center_name = f"InValutazione_{sub_queue}"
                    if center_name in centers:
                        queue_time = invalutazione_stats['queue_time'].get(sub_queue, 0)
                        exec_time = invalutazione_stats['executing_time'].get(sub_queue, 0)
                        
                        centersData[center_name]["queue_time"].append(queue_time)
                        centersData[center_name]["executing_time"].append(exec_time)

    # Debug: Check data sizes
    for center in centers:
        print(f"{center}: {len(centersData[center]['queue_time'])} data points")

    # Find optimal k for all services using powers of 2 from 16 to 128
    k_powers = [16, 32, 64, 128]
    best_k = None
    ...
    for k in k_powers:
        ...
        for service in centers:
            ...
            try:
                ...
                if service in ["InValutazione_Diretta", "InValutazione_Leggera"]:
                    correlation_weight = 3.0  # 3x weight for these priority queues
                    print(f"  {service}: k={k}, |correlation|={abs(correlation_q):.4f} (WEIGHTED 3x), batch_size={b}")
                else:
                    print(f"  {service}: k={k}, |correlation|={abs(correlation_q):.4f}, batch_size={b}")
                
                k_score += (correlation_q ** 2) * correlation_weight
        ...
        if services_with_valid_k > 0:
            ...
            print(f"k={k}: Weighted correlation score = {avg_score:.6f} ({services_with_valid_k} services)")
            print(f"      (InValutazione_Diretta & _Leggera weighted 3x for correlation minimization)")
    ...
    print(f"\nSelected optimal k={best_k} with weighted correlation score = {best_score:.6f}")
    print("(InValutazione_Diretta & InValutazione_Leggera correlations weighted 3x for minimization)")
    ...
    print("\n" + "="*60)
    print("CONFIDENCE INTERVALS (Queue Times Only - Priority Queues)")
    print("="*60)
    ...
    output_dir = "batchMeanGraphsPriority"
    ...
    summary_file = f"{output_dir}/batch_means_priority_summary.txt"
